...
int main(int argc, char **argv) {
    if (argc != 4) {
        const char msg[] = "Usage: child2 <shm_name> <sem_name> <filename>\n";
    ...
    if (shm_fd == -1) {
        const char msg[] = "Child2: error opening shared memory\n";
        write(STDERR_FILENO, msg, sizeof(msg) - 1);
        exit(EXIT_FAILURE);
    }
    ...
    if (shared_data == MAP_FAILED) {
        const char msg[] = "Child2: error mapping shared memory\n";
        write(STDERR_FILENO, msg, sizeof(msg) - 1);
        exit(EXIT_FAILURE);
    }
    ...
    if (semaphore == SEM_FAILED) {
        const char msg[] = "Child2: error opening semaphore\n";
        write(STDERR_FILENO, msg, sizeof(msg) - 1);
        exit(EXIT_FAILURE);
    }
    ...
    if (file == -1) {
        const char msg[] = "Child2: error opening file\n";
        write(STDERR_FILENO, msg, sizeof(msg) - 1);
        exit(EXIT_FAILURE);
    }

    const char start_msg[] = "Child2 started and waiting for data...\n";
    ...
    while (1) {
        ...
        if (shared_data->data_ready && !shared_data->for_child1) {
            ...
            write(file, "\n", 1);
            write(STDOUT_FILENO, "Child2 processed: ", 18);
    ...
    const char finish_msg[] = "Child2 finished.\n";
